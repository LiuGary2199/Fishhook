...
public class CashOutPanel : ShedUIHobby
{
    ...
    private void Start()
    {
        CashOutManager.AgeFletcher()._CashOutPanel = this;
        CloseBtn.onClick.AddListener(() => { BloodUIJazz(nameof(CashOutPanel)); });
        CashOutBtn.onClick.AddListener(() => { OnCashOutBtn(); });
        MoneyName = TedSlumElk.instance.CashOut_Data.MoneyName;
        InfoText.text = TedSlumElk.instance.CashOut_Data.Description;
        InfoPanel_OpenBtn.onClick.AddListener(() => OpenPanel(InfoPanel));
        InfoPanel_CloseBtn.onClick.AddListener(() => ClosePanel(InfoPanel));
        ContactBtn.GetComponent<Text>().text = TedSlumElk.instance.ShedPeg;
        ContactBtn.onClick.AddListener(() =>
        {
            string url = TedSlumElk.instance.ShedPeg;
        ...
        RecordPanel_OpenBtn.onClick.AddListener(() =>
        {
            ...
            CashOutManager.AgeFletcher().GetWithdrawRecord();
        });
        ...
        AccountText.text = CashOutManager.AgeFletcher().Account;
        Confirm_AccountText.text = CashOutManager.AgeFletcher().Account;
        if (string.IsNullOrEmpty(AccountText.text))
        {
            AccountText.text = "Account";
            Confirm_AccountText.text = "Account";
        }
        IDText.text = "UID:" + CashOutManager.AgeFletcher().Data.UserID;
        PolicyBtn.onClick.AddListener(() =>
        {
            string url = TedSlumElk.instance.ShedPeg + "/privacy_policy.html";
#if UNITY_ANDROID || UNITY_EDITOR
            Application.OpenURL(url);
#elif UNITY_IOS
        openUrl(url);
        ...
        PolicyToggle.isOn = SpotGushAwesome.GetBool("CashOut_PolicyAgree");
   ...
   ClanAwesome.Instance.BladeSparsely();
        UpdateUserInfo();
        if (ZJT_Manager.AgeFletcher().DisplayScale != Vector2.zero)
            transform.root.GetComponent<CanvasScaler>().referenceResolution = ZJT_Manager.AgeFletcher().DisplayScale;
    }
    ...
    public override void Hidding()
    {
        ...
        CashOutManager.AgeFletcher().WaitToSendEvent1304();
        if (ZJT_Manager.AgeFletcher().DisplayScale != Vector2.zero)
            transform.root.GetComponent<CanvasScaler>().referenceResolution = ZJT_Manager.AgeFletcher().DisplayScale;
        if (MoteWould.Instance.CruelSmile == 1)
            MoteWould.Instance.Cruel_2();
    }

    public void UpdateUserInfo() //拉取用户信息 显示加载界面
    {
        //StartLoadingAnim(UpdateUI_Loading);
        CashOutManager.AgeFletcher().UpdateUserInfo();
    }
    ...
    {
        if (CashOutManager.AgeFletcher().Data == null)
            return;
        if (string.IsNullOrEmpty(CashOutManager.AgeFletcher().Account))
        {
            OpenPanel(AccountPanel);
            return;
        }
    ...
    void OnConfimBtn() //确认修改账户
    {
        ...
        {
            BrinyAwesome.AgeFletcher().DaleBriny("Account cannot be empty");
            return;
        }
        if (!string.IsNullOrEmpty(AccountErrorText.text))
        {
            BrinyAwesome.AgeFletcher().DaleBriny(AccountErrorText.text);
            return;
        }

        CashOutManager.AgeFletcher().Account = AccountInput.text;
        SpotGushAwesome.SetString("CashOut_Account", AccountInput.text);
    ...
    void OnSbmitBtn() //确认是否同意协议 同意后调用提现接口
    {
        ...
        {
            BrinyAwesome.AgeFletcher().DaleBriny("Please agree to the policy");
            return;
        }
        SpotGushAwesome.SetBool("CashOut_PolicyAgree", true);
        StartLoadingAnim(CashOutBtn_Loading);
        StartLoadingAnim(SbmitBtn_Loading);
        CashOutManager.AgeFletcher().Withdraw();
    }
    ...
    public void UpdateMoney()
    {
        ...
        float MoneyEnd = CashOutManager.AgeFletcher().Money;
        DOTween.To(() => MoneyStart, x => MoneyText.text = x.ToString("F2"), MoneyEnd, 1f);
        CashText.text = CashOutManager.AgeFletcher().Data.Cash.ToString("F2");
        //进度条动画
        float MaxMoney = float.Parse(TedSlumElk.instance.CashOut_Data.convert_goal, CultureInfo.CurrentCulture);
    ...
    public void MoneyToCashAnim()
    {
        ...
        float CashOutEnd = CashOutManager.AgeFletcher().Data.Cash;
    ...
    {
        if (CashOutManager.AgeFletcher().Data.TaskData != null && CashOutManager.AgeFletcher().Data.TaskData.Name != "Null")
        {
            // Cash余额任务 不走前端进度统计 使用后台Data.Cash数值
            if (CashOutManager.AgeFletcher().Data.TaskData.Name == "Cash" && CashOutManager.AgeFletcher().Data.Cash < CashOutManager.AgeFletcher().Data.TaskData.Target)
            {
                CashOutBtn.gameObject.SetActive(false);
                CashOutBtn_Task.SetActive(true);
                CashOutBtn_Task_Text.text = string.Format(CashOutManager.AgeFletcher().Data.TaskData.Description, CashOutManager.AgeFletcher().Data.Cash, CashOutManager.AgeFletcher().Data.TaskData.Target);
            }
            // 其他任务 前端统计任务完成进度 
            else if (CashOutManager.AgeFletcher().Data.TaskData.Name != "Cash" && CashOutManager.AgeFletcher().Data.TaskData.NowValue < CashOutManager.AgeFletcher().Data.TaskData.Target)
            {
                CashOutBtn.gameObject.SetActive(false);
                CashOutBtn_Task.SetActive(true);
                CashOutBtn_Task_Text.text = string.Format(CashOutManager.AgeFletcher().Data.TaskData.Description, CashOutManager.AgeFletcher().Data.TaskData.NowValue, CashOutManager.AgeFletcher().Data.TaskData.Target);
            }
            // 任务完成 显示提现按钮
            else
            {
    ...
    public void UpdateRecord(int Page = 0) //更新单页10个提现记录
    {
        ...
        if (CashOutManager.AgeFletcher().Data.Record == null || CashOutManager.AgeFletcher().Data.Record.Count == 0)
        {
            NoRecordTip.SetActive(true);
            return;
        }

        int RecordCount = CashOutManager.AgeFletcher().Data.Record.Count;
        ...
        List<WithdrawRecordItem> OnePageRecord = CashOutManager.AgeFletcher().Data.Record.GetRange(startIndex, itemsToShow);
    ...
    public void CloseLoading_Withdraw(bool IsCashOutSuccess = false)
    {
        ...
        if (IsCashOutSuccess)
        {
            BrinyAwesome.AgeFletcher().DaleBriny("Withdraw success");
            ClosePanel(ConfirmAccountPanel);
        }
    }
    ...
    {
        QuitCacheCandle.AgeFletcher().HornCache("1301", LeftTimeText.text, CashOutManager.AgeFletcher().Money.ToString());
    }
}
