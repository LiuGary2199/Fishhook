...
public class BuildEditScript : MonoBehaviour
{
    ...
    public static void Check(int buildTargetIndex, BuildInfoData info)
    {
        ...
        if (Resources.FindObjectsOfTypeAll<TedSlumElk>()[0].ShedPeg != info.BaseUrl)
        {
            isSame = false;
            Debug.Log("域名");
        }
        ...
        if (Resources.FindObjectsOfTypeAll<ADAwesome>()[0].MAX_SDK_KEY != info.Applovin_SDK_KEY)
        {
            isSame = false;
            Debug.Log("admanager key");
        }
        if (Resources.FindObjectsOfTypeAll<ADAwesome>()[0].MAX_REWARD_ID != info.Applovin_REWARD_ID)
        {
            isSame = false;
            Debug.Log("reward id");
        }
        if (Resources.FindObjectsOfTypeAll<ADAwesome>()[0].MAX_INTER_ID != info.Applovin_INTER_ID)
        {
            isSame = false;
            Debug.Log("inter id");
        }
        if (Resources.FindObjectsOfTypeAll<TedSlumElk>()[0].ClanCode != info.GameCode)
        {
            isSame = false;
            Debug.Log("gamecode");
        }
    ...
    public static void SuperBuild(int buildTargetIndex, BuildInfoData info)
    {
        ...
        Resources.FindObjectsOfTypeAll<ADAwesome>()[0].MAX_SDK_KEY = info.Applovin_SDK_KEY;
        Resources.FindObjectsOfTypeAll<ADAwesome>()[0].MAX_REWARD_ID = info.Applovin_REWARD_ID;
        Resources.FindObjectsOfTypeAll<ADAwesome>()[0].MAX_INTER_ID = info.Applovin_INTER_ID;
        ...
        if (!GameObject.Find("MainManager") || !GameObject.Find("MainManager").GetComponent<ArouseCapeAwesome>())
        {
            if (GameObject.Find("MainManager"))
            {
                GameObject.Find("MainManager").AddComponent<ArouseCapeAwesome>();
            }
            else
            {
                GameObject mainObj = new GameObject("MainManager");
                mainObj.AddComponent<ArouseCapeAwesome>();
            }
        }
        GameObject.Find("MainManager").GetComponent<ArouseCapeAwesome>().ScriptID = info.Adjust_APP_ID;

        if (!GameObject.Find("MainManager").GetComponent<TrapNoAwesome>())
        {
            GameObject.Find("MainManager").AddComponent<TrapNoAwesome>();
        }

#if UNITY_IOS
        GameObject.Find("MainManager").GetComponent<TrapNoAwesome>().appid = info.Rate_ID;
#endif
#if UNITY_ANDROID
        GameObject.Find("MainManager").GetComponent<TrapNoAwesome>().appid = info.PackageName;
#endif
...
#endif
#if JT
        GameObject.Find("MainManager").GetComponent<JT_Manager>()._LoginPlatform = (LoginPlatform)info.ZT_LoginPlatform;
        GameObject.Find("MainManager").GetComponent<JT_Manager>().AppInfo = info.ZT_ID;
        GameObject.Find("MainManager").GetComponent<JT_Manager>().BaseUrl = modifiedUrl;
#endif

        Resources.FindObjectsOfTypeAll<TedSlumElk>()[0].ClanCode = info.GameCode;
        Resources.FindObjectsOfTypeAll<TedSlumElk>()[0].ShedPeg = info.BaseUrl;
        Resources.FindObjectsOfTypeAll<TedSlumElk>()[0].ShedImplyPeg = info.BaseUrl + CMillet.ImplyPeg;
        Resources.FindObjectsOfTypeAll<TedSlumElk>()[0].ShedMilletPeg = info.BaseUrl + CMillet.MilletPeg;
        Resources.FindObjectsOfTypeAll<TedSlumElk>()[0].ShedDutyPeg = info.BaseUrl + CMillet.DutyPeg;
        Resources.FindObjectsOfTypeAll<TedSlumElk>()[0].ShedArousePeg = info.BaseUrl + CMillet.ArousePeg;
        EditorSceneManager.SaveScene(EditorSceneManager.GetActiveScene());

#if UNITY_IOS
        string url = info.BaseUrl + CMillet.MilletPeg + info.GameCode + "&channel=" + "AppStore" + "&version=" + buildWindow.Version;
#elif UNITY_ANDROID
        string url = info.BaseUrl + CMillet.MilletPeg + info.GameCode + "&channel=" + "GooglePlay" + "&version=" + buildWindow.Version;
#else
        string url = info.BaseUrl + CMillet.MilletPeg + info.GameCode + "&channel=" + "GooglePlay" + "&version=" + buildWindow.Version;
#endif
       ...
       (data) =>
       {
           ...
           //if (rootData.data.apple_pie != "apple")
           {
               ...
               if (GameObject.Find("TedYearAwesome"))
               {
                   DestroyImmediate(GameObject.Find("TedYearAwesome"));
               }
           }
       },
       () =>
