...
public class AudioVolumeEditWindow : EditorWindow
{
    ...
    Dictionary<string, InnerKnife> UIAudioGroup;
    Dictionary<string, InnerKnife> SceneAudioGroup;
    //序列化对象
    ...
    {
        UIAudioGroup = new Dictionary<string, InnerKnife>();
        SceneAudioGroup = new Dictionary<string, InnerKnife>();
        ...
        Dictionary<string, InnerKnife> audioDic = new Dictionary<string, InnerKnife>();
        if (FileInfo.Exists)
        {
        ...
        //UI
        foreach(object music in Enum.GetValues(typeof(ChileSick.UIMusic)))
        {
            string name = music.ToString();
            if (name == "None") continue;
            InnerKnife model = new InnerKnife();
        ...
        // Scene
        foreach (object music in Enum.GetValues(typeof(ChileSick.SceneMusic)))
        {
            string name = music.ToString();
            if (name == "None") continue;
            InnerKnife model = new InnerKnife();
    ...
    public Dictionary<string, InnerKnife> LoadJson(string Filepath)
    {
        StreamReader reader = new StreamReader(Filepath);
        string jsonString = reader.ReadToEnd();
        Dictionary<string, InnerKnife> audioGroup = JsonMapper.ToObject<Dictionary<string, InnerKnife>>(jsonString);
        reader.Dispose();
        reader.Close();
        return audioGroup;
    }
    ...
    {
        Dictionary<string, InnerKnife> dic = new Dictionary<string, InnerKnife>();
        MergeDictionaries.Voter(dic, UIAudioGroup);
        MergeDictionaries.Voter(dic, SceneAudioGroup);
