...
{
    public static void Voter<TKey, TValue>(this IDictionary<TKey, TValue> first, IDictionary<TKey, TValue> second)
    {
...
public class AgeBureauGush : TireStability<AgeBureauGush>
{
    // Start is called before the first frame update
    public bool EvenTumble;
    public bool ToLean;

    static public AgeBureauGush instance;
    ...
    /// <returns></returns>
    public Vector3 RunCivicPetal(GameObject gameobj)
    {
    ...
    /// <returns></returns>
    public GameObject AgeWrapperClanSubway(Transform SelfTransform, List<Transform> listTemp)
    {
    ...
    /// <param name="p2"></param>
    /// <returns></returns>
    public float PetalIDCargo(Vector2 p1, Vector2 p2)
    {
        Vector2 p;
        p.x = p2.x - p1.x;
        p.y = p2.y - p1.y;
    ...
    /// <returns></returns>
    public float Cargo180ID360(float angle)
    {
    ...
    /// <param name="p2"></param>
    /// <returns></returns>
    public int AgeUnityDeformity(Vector2 p1, Vector2 p2)
    {
        float angle = Cargo180ID360(PetalIDCargo(p1, p2));
    ...
    /// <returns></returns>
    public Vector2 RunStationClawFuel(Renderer Renderer)
    {
    ...
    public Vector2 RunStationClawFuel(Image image)
    {
    ...
    /// <param name="obj"></param>
    /// <returns></returns>
    public Vector2 RunMidwayFuel(GameObject obj)
    {
    ...
    /// </summary>
    /// <returns></returns>
    public Vector2 RunFluidCompress()
    {

        Vector3 v = Input.mousePosition;
        float xa = v.x / Screen.width;
        float ya = v.y / Screen.height;
        float screenW = RunRumbleEnure();
        float screenH = RunRumbleCanopy();
        float x = -screenW / 2 + xa * screenW;
        float y = -screenH / 2 + ya * screenH;
        return new Vector2(x, y);
    }

    public int RunBounceSmile(List<string> weightList)
    {
        float sumWeight = 0;
    ...
    public int[] AgePropelBasaltic(int total, int count)
    {
    ...
    public Dictionary<string, object> TankerIDUncritical(string value)
    {
        if (value.Length < 1)
        {
            return null;
    ...
    public string UncriticalIDTanker(Dictionary<string, object> Info)
    {
    ...
    public int DayWildTusk(string dateStart, DateTime dateEnd)
    {
    ...
    public int DogWildTusk(string dateStart, DateTime dateEnd)
    {
    ...
    public TimeSpan WildTusk(DateTime dateStart, DateTime dateEnd)
    {
    ...
    public Vector3 AgeSheOnWelt(Canvas canvas, Text text, string strFragment)
    {
        int strFragmentIndex = text.text.IndexOf(strFragment);//-1表示不包含strFragment
        Vector3 stringPos = Vector3.zero;
        if (strFragmentIndex > -1)
        {
            Vector3 firstPos = AgeSheOnWelt(canvas, text, strFragmentIndex);
            Vector3 lastPos = AgeSheOnWelt(canvas, text, strFragmentIndex + strFragment.Length);
            stringPos = (firstPos + lastPos) * 0.5f;
        }
        else
        {
            stringPos = AgeSheOnWelt(canvas, text, strFragmentIndex);
        }
        return stringPos;
    }

    public Vector3 AgeSheOnWelt(Canvas canvas, Text text, int charIndex)
    {
    ...
    /// <returns></returns>
    public float FireballMeTill(Vector3 pB, Vector3 pA1, Vector3 pA2)
    {
    ...
    private static byte[] King= { 0x12, 0x34, 0x56, 0x78, 0x90, 0xAB, 0xCD, 0xEF };
    /// <summary>
    /// DES加密字符串
    /// </summary>
    ...
    public static string ChieflyDES(string encryptString, string key = "abcdefgh")
    {
        try
        {
    ...
    public static string CleanlyDES(string decryptString, string key = "13717421")
    {
        byte[] rgbKey = Encoding.UTF8.GetBytes(Application.identifier.Substring(0, 8));
        byte[] rgbIV = King;
