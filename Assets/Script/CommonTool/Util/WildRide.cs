...
{
    public static long Reliant()
    {
        return AgeHusbandry(DateTime.Now);
    }

    public static long AgeHusbandry(DateTime datetime)
    {
    ...
    /// <returns></returns>
    public static string WildDutyFormat(DateTime dt, string format)
    {
        return dt.ToString(format);
    }

    public static DateTime HemlockIDWildDuty(long seconds)
    {
        DateTime startTime = TimeZone.CurrentTimeZone.ToLocalTime(new DateTime(1970, 1, 1)); // 当地时区
    ...
    /// <returns></returns>
    public static string HemlockEntire(long totalTime, string connector = ":")
    {
    ...
    /// <param name="totalTime"></param>
    /// <returns></returns>
    public static string HemlockEntire2(long totalTime)
    {
        int seconds = Mathf.Max((int)(totalTime % 60), 0);
        int minutes = Mathf.Max((int)(totalTime / 60) % 60, 0);
    ...
    //根据权重和范围获取奖励数据
public static RewardData AgeLessonGushMeBounceOffRadio(List<RewardData> rewardDatas, bool UseRandom = true)
{
    int totalWeight = 0;
    ...
    foreach (RewardData item in rewardDatas)
    {
        ...
        {
            if (PotionUtil.AxApple() && item.type == RewardType.Cash)
                item.type = RewardType.Cash;

            if (item.type == RewardType.Cash)
            {
                item.rewardNum = UnityEngine.Random.Range(item.numMin, item.numMax);
                item.rewardNum *= (float)GameUtil.GetGoldMulti();
            }
            else if (item.type == RewardType.Diamond)
            {
                item.rewardNum = (float)System.Math.Round(UnityEngine.Random.Range((float)item.numMin, (float)item.numMax), 2);
                item.rewardNum *= (float)GameUtil.GetCashMulti();
                item.rewardNum = (float)System.Math.Round(item.rewardNum, 2);
            }
            return item;
        }
    }
